...
contract NFTMarketTest is Test {
    ...
    function test_ListNFT() public {
        ...
        (address listingSeller, uint64 price, bool active) = market.getListing(0);
    ...
    function test_RevertWhen_ListNFTNotApproved() public {
        ...
        vm.expectRevert(NFTMarket.NFTNotApprovedToMarket.selector);
        market.list(0, 100);
    ...
    function test_RevertWhen_PermitBuyExpiredDeadline() public {
        ...
        vm.expectRevert(NFTMarket.PermitBuyExpiredDeadline.selector);
        market.permitBuy(0, 100, deadline, v, r, s);
    ...
    function test_RevertWhen_PermitBuyInvalidSignature() public {
        ...
        // 使用错误的签名
        vm.expectRevert(NFTMarket.PermitBuyInvalidSignature.selector);
    ...
    function test_RevertWhen_PermitBuyPriceMismatch() public {
        ...
        vm.startPrank(seller);
        nft.approve(address(market), 0);
        market.list(0, uint64(200)); // 价格是 200，但签名是 100
        vm.stopPrank();

        // 买家 approve 代币
        vm.startPrank(buyer);
        token.approve(address(market), uint64(200));

        vm.expectRevert(NFTMarket.PriceMismatch.selector);
        market.permitBuy(0, uint64(100), deadline, v, r, s);
    ...
    function test_Events() public {
        ...
        emit NFTMarket.NFTListed(0, seller, uint64(100));
        market.list(0, uint64(100));
        vm.stopPrank();
        ...
        vm.startPrank(buyer);
        token.approve(address(market), uint64(100));

        vm.expectEmit(true, true, true, true);
        emit NFTMarket.PermitBuyExecuted(0, buyer, seller, uint64(100));
        market.permitBuy(0, 100, deadline, v, r, s);
