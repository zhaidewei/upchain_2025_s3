...
SIGNATURE_OUTPUT=$(npx tsx src/index.ts sign-permit \
  ...
  --deadline $DEADLINE \
  --chain-id $CHAIN_ID \
  --contract-address $ERC20_CONTRACT)

echo "$SIGNATURE_OUTPUT"
echo ""

# 提取签名参数
V=$(echo "$SIGNATURE_OUTPUT" | grep "^v: " | sed 's/v: //')
R=$(echo "$SIGNATURE_OUTPUT" | grep "^r: " | sed 's/r: //')
S=$(echo "$SIGNATURE_OUTPUT" | grep "^s: " | sed 's/s: //')
